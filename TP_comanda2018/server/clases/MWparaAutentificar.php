...
public function VerificarMozo($request, $response, $next) {
	...
	if ($objDelaRespuesta->esValido){
		...
		if (strtolower($payload->tipo)=="mozo"){
			  $response = $next($request, $response);
		} 
		else{
			$objDelaRespuesta->respuesta="Acceso permitido solo a mozos";
			return $response->withJson($objDelaRespuesta, 403);
		  }
	} 
...
public function VerificarEmpleado($request, $response, $next) {
	...
	if ($objDelaRespuesta->esValido){
		...
		if ((strtolower($payload->tipo)=="cocinero") or (strtolower($payload->tipo)=="cervecero") or (strtolower($payload->tipo)=="pastelero") or (strtolower($payload->tipo)=="bartender"))
		{
			  $response = $next($request, $response);
		} 
		else{
			$objDelaRespuesta->respuesta="Acceso permitido solo a personal de preparación de pedido";
			return $response->withJson($objDelaRespuesta, 403);
		  }
	} 
