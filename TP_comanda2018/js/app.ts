...
function TSMostrarGrillaEmpleados() {
    $.ajax({
        ...
        let empAux: restaurante.empleado;
        let listaEmpleados = [];
        ...
        */
        ...
        tCabeza.html("");
        tCuerpo.html("");
        let msjEmpleado = $("#msjEmpleado").html("<h4 id='msjEmpleado'><kbd> Lista de Usuarios</kbd></h2>");
...
function filtrarAdmin() {
    $.ajax({
        ...
        if (tipoFiltrado != "todos") {
            ...
            empleadosFiltrados = aux.filter(function (emp: restaurante.empleado) {
        ...
        //Si la opción es "Todos", llama a mostrar grilla enviando todos los datos recibidos mediante GET.
            ...
            //caso de error de obtención de datos
                ...
                let msjVehiculo = $("#msjEmpleado").html("<h4 id='msjEmpleado'><kbd>" + aux + "</kbd></h2>");
                console.log(aux);
            }
        }
...
function TSidEmpleado(auxEmpleados: any[]) {
    let chIDUsuario = $("#chIDUsuario");
    let lID: string;
...
function TSemailEmpleado(auxEmpleados: any[]) {
    let chEmail = $("#chEmail");
    let lMail: string;
...
function TStipoEmpleado(auxEmpleados: any[]) {
    let chTipo = $("#chTipo");
...
function TSestadoEmpleado(auxEmpleados: any[]) {
    let chEstado = $("#chEstado");
    let lEstado: string;
...
function ocultarLabel(id:string)
{
    if(id != null)
    {
...
function TSMostrarGrillaPedidos() {
    ...
    $.ajax({
        ...
        Filtrar Admin para que muestre/oculte la columna seleccionada en la grilla.
        */
       let chIDComanda = $("#chIDComanda");
       let chHoraEstimada = $("#chHoraEstimada");
       let chHoraFinal = $("#chHoraFinal");
       let chIDEmpleadop = $("#chIDEmpleadop");
       let chMonto = $("#chMonto");
       let chFecha = $("#chFecha");
        ...
        tCabeza.html("");
        tCuerpo.html("");
        let msjEmpleado = $("#msjEmpleado").html("<h4 id='msjEmpleado'><kbd> Lista de Pedidos</kbd></h2>");
        tCabeza.append("<tr class='success'>" +
            "<th> ID</th>" +
            "<th> ID Comanda</th>" +
            "<th> ID Producto</th>" +
...
function TSMostrarGrillaPedidosFiltrada(auxPedidos: any) {
    let chIDComanda = $("#chIDComanda");
    let chHoraEstimada = $("#chHoraEstimada");
    let chHoraFinal = $("#chHoraFinal");
    let chIDEmpleadop = $("#chIDEmpleadop");
    ...
    let auxIDComanda: any;
    let auxHoraEstimada: any;
    let auxHoraFinal: any;
    let auxIDEmpleadop: any;
    let auxMonto: any;
    let auxFecha: any;
    ...
    */
    for (var index = 0; index < auxPedidos.length; index++) {
        let mapIDComanda = TSidComanda(auxPedidos);
        let mapHoraEstimada = TShoraEstimada(auxPedidos);
        let mapHoraFinal = TShoraFinal(auxPedidos);
        let mapIDEmpleadop = TSidEmpleadop(auxPedidos);
        let mapMonto = TSMonto(auxPedidos);
        let mapFecha = TSFecha(auxPedidos);
        //append agrega mas al html
        tCuerpo.append(
            "<tr>" +
...
function filtrarAdminPedidos() {
    $.ajax({
        ...
        let pedAux: restaurante.pedido;
        let listaPedidos = [];
        let tipoFiltrado: any = String($("#cbFiltop").val());
        ...
        if (tipoFiltrado != "todoss") {
            ...
            pedidosFiltrados = aux.filter(function (ped: restaurante.pedido) {
...
function filtrosPedidos() {
    $("#sTablaEmpleados").show();
    $("#filtrosEmpleados").show();
    let filtrosPedidos = $("#filtrosEmpleados").html("");
...
function TSidComanda(auxPedidos: any[]) {
    let chIDComanda = $("#chIDComanda");
    let lIDComanda: string;
...
function TShoraEstimada(auxPedidos: any[]) {
    let chHoraEstimada = $("#chHoraEstimada");
    let lHoraEstimada: string;
...
function TShoraFinal(auxPedidos: any[]) {
    let chHoraFinal = $("#chHoraFinal");
    let lHoraFinal: string;
...
function TSidEmpleadop(auxPedidos: any[]) {
    let chIDEmpleadop = $("#chIDEmpleadop");
    let lidEmpleadop: string;
...
function TSMonto(auxPedidos: any[]) {
    let chMonto = $("#chMonto");
    let lMonto: string;
...
function TSFecha(auxPedidos: any[]) {
    let chFecha = $("#chFecha");
    let lFecha: string;
...
function modificarPedido(idPedido: any) {
...
function finalizarPedido(idPedido: any) {
    let form_data = new FormData();
    form_data.append('idPedido', idPedido);
    $.ajax({
        type: "POST",
